"use client"
import React, { useEffect, useRef, useState } from "react";
import { Button } from "@/components/ui/Button";

interface LinkItem {
  href: string;
  label: string;
}

interface FooterProps {
  leftLinks: LinkItem[];
  rightLinks: LinkItem[];
  copyrightText: string;
  barCount?: number; 
  headline?: string;
  subheadline?: string;
  ctaText?: string;
}

export const AnimatedFooter: React.FC<FooterProps> = ({
  leftLinks,
  rightLinks,
  copyrightText,
  barCount = 45, // Increased for a larger wave
  headline,
  subheadline,
  ctaText
}) => {
  const waveRefs = useRef<(HTMLDivElement | null)[]>([]);
  const footerRef = useRef<HTMLDivElement | null>(null);
  const [isVisible, setIsVisible] = useState(false);
  const animationFrameRef = useRef<number | null>(null);

  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        const [entry] = entries;
        setIsVisible(entry.isIntersecting);
      },
      { threshold: 0.1 } 
    );

    if (footerRef.current) {
      observer.observe(footerRef.current);
    }

    return () => {
      if (footerRef.current) {
        observer.unobserve(footerRef.current);
      }
    };
  }, []);

  useEffect(() => {
    let t = 0; 

    const animateWave = () => {
      const waveElements = waveRefs.current;
      let offset = 0;

      waveElements.forEach((element, index) => {
        if (element) {
          // Maintaining the exact logic, just applied to more bars
          offset += Math.max(0, 20 * Math.sin((t + index) * 0.3)); 
          element.style.transform = `translateY(${index + offset}px)`;
        }
      });

      t += 0.1;
      animationFrameRef.current = requestAnimationFrame(animateWave);
    };

    if (isVisible) {
      animateWave();
    } else if (animationFrameRef.current) {
      cancelAnimationFrame(animationFrameRef.current);
      animationFrameRef.current = null;
    }

    return () => {
      if (animationFrameRef.current) {
        cancelAnimationFrame(animationFrameRef.current);
        animationFrameRef.current = null;
      }
    };
  }, [isVisible]);

  return (
    <footer
      ref={footerRef}
      className="bg-black text-white relative flex flex-col w-full h-full justify-between min-h-screen select-none overflow-hidden"
    >
      {/* SellQ Intelligence Nexus Content */}
      <div className="container mx-auto flex flex-col items-center justify-center text-center w-full px-4 pt-32 pb-16 z-10 flex-1">
        {headline && (
          <h2 className="text-5xl md:text-7xl font-bold tracking-tighter mb-6 bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-blue-500 to-cyan-400">
            {headline}
          </h2>
        )}
        {subheadline && (
          <p className="text-lg md:text-xl text-neutral-400 font-light max-w-2xl mb-12">
            {subheadline}
          </p>
        )}
        {ctaText && (
          <Button className="h-14 px-10 rounded-full bg-white text-black hover:bg-neutral-200 text-lg font-semibold transition-all hover:scale-105 active:scale-95 shadow-[0_0_40px_rgba(59,130,246,0.3)]">
            {ctaText}
          </Button>
        )}
      </div>

      {/* Links & Copyright */}
      <div className="container mx-auto flex flex-col md:flex-row justify-between w-full gap-8 pb-12 pt-8 px-8 z-10">
        <div className="space-y-4">
          <ul className="flex flex-wrap gap-x-6 gap-y-3">
            {leftLinks.map((link, index) => (
              <li key={index}>
                <a href={link.href} className="text-sm font-medium text-neutral-400 hover:text-cyan-400 transition-colors">
                  {link.label}
                </a>
              </li>
            ))}
          </ul>
          <p className="text-sm mt-4 flex items-center gap-x-2 text-neutral-500">
            <svg className="size-4 text-purple-500" viewBox="0 0 80 80">
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                fill="currentColor"
                d="M67.4307 11.5693C52.005 -3.85643 26.995 -3.85643 11.5693 11.5693C-3.85643 26.995 -3.85643 52.005 11.5693 67.4307C26.995 82.8564 52.005 82.8564 67.4307 67.4307C82.8564 52.005 82.8564 26.995 67.4307 11.5693ZM17.9332 17.9332C29.8442 6.02225 49.1558 6.02225 61.0668 17.9332C72.9777 29.8442 72.9777 49.1558 61.0668 61.0668C59.7316 62.4019 58.3035 63.5874 56.8032 64.6232L56.8241 64.6023C46.8657 54.6439 46.8657 38.4982 56.8241 28.5398L58.2383 27.1256L51.8744 20.7617L50.4602 22.1759C40.5018 32.1343 24.3561 32.1343 14.3977 22.1759L14.3768 22.1968C15.4126 20.6965 16.5981 19.2684 17.9332 17.9332ZM34.0282 38.6078C25.6372 38.9948 17.1318 36.3344 10.3131 30.6265C7.56889 39.6809 9.12599 49.76 14.9844 57.6517L34.0282 38.6078ZM21.3483 64.0156C29.24 69.874 39.3191 71.4311 48.3735 68.6869C42.6656 61.8682 40.0052 53.3628 40.3922 44.9718L21.3483 64.0156Z"
              />
            </svg>
            {copyrightText}
          </p>
        </div>
        <div className="space-y-4">
          <ul className="flex flex-wrap gap-x-6 gap-y-3 justify-start md:justify-end">
            {rightLinks.map((link, index) => (
              <li key={index}>
                <a href={link.href} className="text-sm font-medium text-neutral-400 hover:text-purple-400 transition-colors">
                  {link.label}
                </a>
              </li>
            ))}
          </ul>
          <div className="text-left md:text-right mt-4">
            <button 
              onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
              className="text-sm text-neutral-500 hover:text-white hover:underline transition-colors"
            >
              Back to top
            </button>
          </div>
        </div>
      </div>

      {/* Wave Animation */}
      <div
        id="waveContainer"
        aria-hidden="true"
        className="absolute bottom-0 left-0 w-full z-0 opacity-80"
        style={{ overflow: "hidden", height: 250 }}
      >
        <div style={{ marginTop: 0 }} className="flex flex-col items-center justify-center">
          {Array.from({ length: barCount }).map((_, index) => {
             // Gradient pattern: Clean White / Silver
             const colors = ["#ffffff", "#f3f4f6", "#e5e7eb", "#d1d5db", "#9ca3af"];
             const bgColor = colors[index % colors.length];
             
             return (
              <div
                key={index}
                ref={(el) => { waveRefs.current[index] = el; }}
                className="wave-segment w-full"
                style={{
                  height: `${Math.pow((index + 1), 1.2)}px`, // Exponential height for steeper curve
                  backgroundColor: bgColor,
                  transition: "transform 0.1s ease",
                  willChange: "transform",
                  marginTop: "-1px", // Seamless connection
                  boxShadow: `0 0 10px ${bgColor}40`
                }}
              />
            );
          })}
        </div>
      </div>
    </footer>
  );
};

export default AnimatedFooter;
